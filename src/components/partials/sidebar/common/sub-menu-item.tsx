  ...
  if (subItem.badge) {
    return (
      <span className="text-sm flex space-x-3 items-center transition-all duration-150 opacity-50  cursor-not-allowed">
        <span
          className={` h-2 w-2 rounded-full border border-zinc-600 inline-block flex-none`}
        ></span>
        <div className="flex-1 truncate  flex  text-zinc-600">
...
const SubMenuItem = ({ subItem }) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'text-sm  font-normal flex gap-3 items-center transition-all duration-150 rounded  ',
          {
