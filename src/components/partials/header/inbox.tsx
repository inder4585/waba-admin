    ...
    <DropdownMenu>
      ...
      >
        ...
        >
          ...
          <Link
            href="/view-message"
            className="text-xs font-medium text-white flex-0 hover:underline hover:decoration-default-100"
          >
            View All
          </Link>
        </DropdownMenuLabel>
        <div className="h-[350px] xl:h-[420px]">
          <ScrollArea className="h-full">
            {contacts.map((item, index) => (
              <DropdownMenuItem
                key={`inbox-${index}`}
                className="flex gap-9 py-2 px-4 cursor-pointer rounded-none"
              >
