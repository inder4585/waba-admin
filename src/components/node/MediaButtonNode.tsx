...
import { MdPermMedia, MdEdit, MdDelete } from 'react-icons/md';

const MediaButtonNodeComponent = ({ id, data, isConnectable }: NodeProps<MediaButtonNode>) => {
  ...
  return (
    <div className="shadow-lg rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 min-w-[280px] overflow-visible transition-all hover:shadow-xl hover:scale-[1.02]">
      ...
      <div className="p-0">
        {/* Media Preview Area */}
        <div className="bg-gray-100 dark:bg-slate-900 h-32 flex items-center justify-center relative overflow-hidden border-b border-gray-50 dark:border-slate-700">
            {mediaUrl ? (
                mediaType === 'image' ? (
                    <img src={mediaUrl} alt="Preview" className="w-full h-full object-cover" />
                ) : (
                    <div className="text-gray-400 dark:text-gray-500 text-xs font-bold uppercase">{mediaType} PREVIEW</div>
                )
            ) : (
                <div className="flex flex-col items-center gap-2 text-gray-300 dark:text-gray-600">
                    <MdPermMedia className="w-8 h-8 opacity-20" />
                    <span className="text-[10px] font-bold uppercase tracking-tighter">No Media Selected</span>
                </div>
            )}
        ...
        <div className="p-5 space-y-4">
            ...
            <div className="flex flex-col gap-3">
                {buttonsArray.map((btn, index) => (
                    <div key={btn.id || index} className="relative group">
                        <div className="bg-white dark:bg-slate-700 text-rose-600 dark:text-rose-400 px-4 py-2.5 rounded-xl text-xs font-bold text-center border-2 border-rose-50 dark:border-rose-900/30 shadow-sm hover:border-rose-200 dark:hover:border-rose-700 transition-all uppercase tracking-wide">
                            {btn.title}
                        </div>
                        <Handle
                            type="source"
                            ...
                            className="w-3 h-3 bg-rose-400 border-2 border-white dark:border-slate-800 !-right-1.5 hover:scale-125 transition-transform"
                        />
                    </div>
                ))}
                {!buttonsArray.length && (
                    <div className="text-[10px] text-gray-300 dark:text-gray-500 text-center py-2 font-bold uppercase tracking-widest border border-dashed border-gray-100 dark:border-slate-700 rounded-lg italic">
                        No buttons added
                    </div>
                )}
            </div>
            ...
            <div className="pt-3 border-t border-dashed border-gray-100 dark:border-slate-700 space-y-2">
                <div className="flex items-center gap-2">
                    <div className="text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase">Store Selection in:</div>
                    <div className="text-[10px] text-blue-600 dark:text-blue-400 font-mono font-bold bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-800/50">[[{saveAsVar}]]</div>
                </div>
            </div>
        </div>
      </div>
