    ...
    <div className="shadow-lg rounded-2xl bg-white border border-gray-100 min-w-[280px] overflow-visible transition-all hover:shadow-xl hover:scale-[1.02]">
      ...
      <div className="p-4 space-y-3">
        ...
        {data?.payload?.body && Object.keys(data.payload.body).length > 0 && (
          <div className="text-[10px] text-gray-400 font-mono bg-gray-900/5 p-2 rounded border border-dashed border-gray-200">
      ...
      <Handle
        ...
        className="w-3 h-3 bg-gray-400 border-2 border-white !-left-1.5 transition-colors hover:bg-cyan-500"
      />
      <Handle
        type="source"
        position={Position.Right}
        isConnectable={isConnectable}
        className="w-3 h-3 bg-cyan-500 border-2 border-white !-right-1.5 transition-colors hover:scale-125"
      />
    </div>
  );
};
