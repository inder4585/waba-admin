...
const MediaNodeComponent = ({
  ...
  return (
    <div className="shadow-lg rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 min-w-[260px] max-w-[280px] overflow-visible transition-all hover:shadow-xl hover:scale-[1.02]">
      {/* Header */}
      ...
      <div className="p-0">
        {/* Media Preview Area */}
        <div className="bg-gray-100 dark:bg-slate-900 h-36 flex items-center justify-center relative overflow-hidden border-b border-gray-50 dark:border-slate-700">
          {mediaUrl ? (
            mediaType === 'image' ? (
              <img
                src={mediaUrl}
        ...
        <div className="p-4 space-y-3">
          {/* Caption */}
          {caption ? (
            <div className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed font-medium bg-gray-50 dark:bg-slate-900/50 p-2 rounded-lg border border-gray-100 dark:border-slate-700">
              {caption}
            </div>
          ) : (
            <div className="text-xs text-gray-400 dark:text-gray-500 italic text-center py-1 border border-dashed border-gray-100 dark:border-slate-700 rounded-lg">
              No caption provided
            </div>
          )}
        </div>
      ...
      <Handle
        ...
        className="w-3 h-3 bg-gray-400 border-2 border-white dark:border-slate-800 !-left-1.5 transition-colors hover:bg-fuchsia-500"
      />
      <Handle
        type="source"
        ...
        id="next"
        isConnectable={isConnectable}
        className="w-3 h-3 bg-fuchsia-400 border-2 border-white dark:border-slate-800 !-right-1.5 transition-colors hover:scale-125"
      />
    </div>
  );
};
